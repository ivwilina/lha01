...
const QuizComprehensive = () => {
  ...
  useEffect(() => {
    ...
    const createQuiz = async () => {
      try {
        ...
        const newQuiz = await createComprehensiveQuiz(currentUser._id, numQuestions);
        setQuiz(newQuiz);
        ...
        console.error('Error creating comprehensive quiz:', err);
        setError(err.message || 'Không thể tạo quiz tổng hợp');
      } finally {
        setLoading(false);
      }
    };
    ...
    createQuiz();
  }, [currentUser, numQuestions, isAuthenticated, navigate]);
  ...
  const handleSubmitQuiz = useCallback(async () => {
    try {
      ...
      const formattedAnswers = Object.entries(answers).map(([questionId, selectedOption]) => {
        ...
        if (question && (question.type === 'fill_blank' || question.type === 'complete_word')) {
      ...
      // Submit quiz (no specific categoryId for comprehensive quiz)
      const quizResults = await submitQuiz(quiz._id, formattedAnswers, currentUser._id, null);
  ...
  if (showResults && results) {
    return (
      <QuizResults 
        results={results}
        category={{ categoryTopic: 'Comprehensive Quiz - All Learned Words' }}
    ...
    <div className="quiz-layout">
      ...
      <div className="quiz-main-content">
        <div className="quiz-header">
          ...
          <p className="quiz-description">All learned words from every category</p>
        </div>
...
export default QuizComprehensive;
